...
type TabId = "today" | "timeline" | "health" | "sleep" | "settings" | "compare" | "share" | "tolerance" | "insights"
...
export default function Home() {
  ...
  const [popupMode, setPopupMode] = React.useState<"calculator" | "custom">("calculator")
  ...
  const { getEntriesToday, getTotalCaffeineToday, getDailyProgress, getCurrentCaffeineLevel, removeEntry, settings, entries } = useCaffeineStore()
  ...
  const navTabs: TabConfig[] = [
    { id: "today", icon: <Sun className="h-5 w-5" />, label: "วันนี้", badge: totalCaffeine > 0 ? `${Math.round(totalCaffeine)}` : undefined },
    { id: "timeline", icon: <History className="h-5 w-5" />, label: "ประวัติ", badge: todayCount > 0 ? todayCount : undefined },
    { id: "health", icon: <Activity className="h-5 w-5" />, label: "สุขภาพ" },
    { id: "sleep", icon: <Moon className="h-5 w-5" />, label: "การนอน" },
    { id: "settings", icon: <Settings className="h-5 w-5" />, label: "ตั้งค่า" },
  ]

  // More tabs (accessible via settings or swipe)
  const moreTabs: TabConfig[] = [
    { id: "compare", icon: <GitCompareArrows className="h-4 w-4" />, label: "เปรียบเทียบ" },
    { id: "share", icon: <Share2 className="h-4 w-4" />, label: "แชร์" },
    { id: "tolerance", icon: <Shield className="h-4 w-4" />, label: "Tolerance" },
    { id: "insights", icon: <Sparkles className="h-4 w-4" />, label: "Insights" },
  ]
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "today":
        return (
          <TodayDashboard 
            onOpenAddPopup={(mode) => { setPopupMode(mode); setShowAddPopup(true) }}
            showChart={showChart}
  ...
  function TodayDashboard({ 
    ...
    const percentage = Math.min((totalCaffeine / dailyProgress.limit) * 100, 100)
      ...
      <div className="space-y-5">
        ...
        <Card className="warm-card overflow-hidden border-0">
          <CardContent className="p-5 sm:p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h2 className="text-lg font-bold text-foreground">กำลังบริโภค:วันนี้</h2>
              ...
              <div className="relative">
                <svg width="160" height="160" className="progress-ring">
                  ...
                  <circle
                    ...
                    r="72"
                    fill="none"
                    stroke={dailyProgress.status === 'exceeded' ? '#DC2626' : '#B8860B'}
                    strokeWidth="10"
                    strokeLinecap="round"
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className={`text-4xl sm:text-5xl font-bold ${
                    dailyProgress.status === 'exceeded' ? 'text-red-500' : 'text-warm-500'
                  }`}>
                    {Math.round(totalCaffeine)}
                  </span>
                  <span className="text-sm text-muted-foreground mt-1">มิลลิกรัม</span>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            <CardContent className="p-4 flex flex-col items-center text-center gap-2">
              ...
              <span className="font-medium text-sm text-foreground mt-1">เลือกเครื่องดื่ม</span>
              <span className="text-xs text-muted-foreground">กาแฟ ชา มัทฉะ</span>
            </CardContent>
          </Card>
          ...
          >
            <CardContent className="p-4 flex flex-col items-center text-center gap-2">
              ...
              <span className="font-medium text-sm text-foreground mt-1">กรอกเอง</span>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                >
                  ...
                  <ChevronRight className={`h-4 w-4 transition-transform ${showEntriesList ? 'rotate-90' : ''}`} />
                </button>
              </div>
              ...
              {showEntriesList ? (
                <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-200">
                  {todayEntries.map((entry, index) => (
                    <div key={entry.id} className={`flex items-center gap-3 p-3 rounded-xl bg-background border border-border/30 group hover:border-warm-300 transition-all ${index === 0 ? 'ring-2 ring-warm-200' : ''}`}>
                      <span className="text-xl">{entry.drink.icon}</span>
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-sm truncate">{entry.drink.nameTh}</p>
                        <p className="text-xs text-muted-foreground">
                          {new Date(entry.timestamp).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge variant="secondary" className="bg-primary/10 text-primary font-mono font-bold">
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r from-warm-50 to-orange-50/50 dark:from-warm-900/20 dark:to-orange-900/10 border border-warm-200/30">
                  <span className="text-2xl">{todayEntries[0]?.drink.icon}</span>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm truncate">{todayEntries[0]?.drink.nameTh}</p>
                    <p className="text-xs text-muted-foreground">
                      ล่าสุด • {new Date(todayEntries[0]?.timestamp || 0).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}
                    </p>
                  </div>
            ...
            <CardContent className="p-4">
              ...
              <div className="relative h-2 bg-white/60 dark:bg-black/20 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${Math.min((currentLevel / totalCaffeine) * 100, 100)}%` }}
                />
              </div>
        ...
        {(showChart && lastAddedAmount > 0) || totalCaffeine > 0 ? (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <CaffeineChart caffeineAmount={Math.max(lastAddedAmount, totalCaffeine)} showCurrentTime={true} />
          </div>
        ) : null}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-foreground text-base">รู้จักคาเฟอีน</h3>
            <button 
          ...
          <Card className="warm-card overflow-hidden border-0 bg-gradient-to-br from-warm-50 to-warm-100 dark:from-warm-900/20 dark:to-warm-800/20 cursor-pointer hover:scale-[1.01] transition-transform" onClick={() => setActiveTab('health')}>
            <CardContent className="p-4">
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-1 line-clamp-2">
                    สารกระตุ้นระบบประสาทส่วนกลาง พบในกาแฟ ชา โกโก้ ช่วยเพิ่มความตื่นและสมาธิ
                  </p>
                </div>
              </div>
